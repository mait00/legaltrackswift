...
const SouCaseView = ({data, action, index, onShowMenu}) => {

	const renderSide = (style, icon, sides) => {

		let str = '';

		for (let i = 0; i < sides.length; i++) {
			str = str + sides[i].nameSide;
			if (i + 1 < sides.length) {
				str = str + ', ';
			}
		}
			...
			<View style={{
				...
				<View style={{
					...
					<View style={{
						...
						{data.status === 'loading' ? (<Image source={require('./../../assets/ic-clock-glass.png')}
							style={{
						...
						<Text style={{
							marginLeft: 2,
							color: data.status === 'loading' ? 'rgba(66, 67, 71, 0.5)' : colors.MAIN_COLOR,
							fontFamily: 'SFProDisplay-Regular',
						...
						{data.status === 'loading' ? (<Text style={{
							marginLeft: 2,
